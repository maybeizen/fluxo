...
const isTicketBanned = async (
    ...
    try {
        ...
        if (punishment.isTicketBanned) {
            return res.status(403).json({
                success: false,
                message:
                    'You are banned from creating or replying to support tickets. Please contact support if you believe this is an error.',
            })
        }

        next()
    } catch (error: unknown) {
        logger.error(`Error in isTicketBanned middleware - ${error}`)
        res.status(500).json({
            success: false,
...
export { isTicketBanned }
export default isTicketBanned
