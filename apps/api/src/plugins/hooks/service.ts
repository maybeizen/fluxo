...
import type { ProvisionServiceResult } from '@fluxo/forge'
 ...
 */
/**
 * Optional overrides for plugin config (e.g. from configurable options at checkout).
 * These override the product integration's servicePluginConfig when provisioning.
 */
export async function provisionServiceWithPlugin(input: {
    productId: number
    serviceName: string
    userId: number
    /** Customer/configurable option overrides; merged over product integration config */
    ...
    const plugin = await manager.getService(integration.servicePluginId)
    if (!plugin) {
        logger.warn(
    ...
    const pluginConfig =
        input.configOverrides && Object.keys(input.configOverrides).length > 0
            ? { ...baseConfig, ...input.configOverrides }
            : baseConfig
    ...
    try {
        ...
        logger.error(
            `Service plugin ${integration.servicePluginId} provision failed: ${message}`,
            { source: 'ServiceHooks' }
        )
        throw err
    }
