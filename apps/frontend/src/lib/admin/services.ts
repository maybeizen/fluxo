...
import { type Service, type ServiceStatus } from '@fluxo/types'
import { type ServiceWithOwner } from '@/components/admin/services/service-table'
...
export async function fetchServiceById(id: string): Promise<Service> {
    const response = await apiClient.get(`/admin/services/id/${id}`, {})
