...
const CHECKOUT_CONFIGURABLE_KEY = 'fluxo:checkout:configurable'
...
export default function CheckoutSummaryPage() {
    ...
    const handleCreateInvoice = async () => {
        ...
        try {
            ...
            let configurableSelections: Record<number, unknown> | undefined
            try {
                const stored = sessionStorage.getItem(CHECKOUT_CONFIGURABLE_KEY)
                if (stored) {
                    ...
                    configurableSelections = parsed.configurableSelections
                }
            } catch {
                // ignore
            }
            ...
            const invoiceData: CheckoutInvoiceData = {
                ...
                currency: 'usd',
                ...(gatewayPluginId ? { gatewayPluginId } : {}),
                metadata: {
                ...
                Object.keys(configurableSelections).length > 0
                    ? { productId, configurableSelections }
                    : {}),
            }
            ...
            if (result.success && result.invoice) {
                ...
                try {
                    sessionStorage.removeItem(CHECKOUT_CONFIGURABLE_KEY)
                } catch {
                    // ignore
                }
                ...
                const invoiceId =
                    result.invoice.uuid ??
                    (result.invoice as { id?: number }).id?.toString()
                if (invoiceId) {
