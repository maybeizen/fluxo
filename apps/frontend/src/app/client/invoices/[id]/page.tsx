...
import { type Invoice, InvoiceStatus, PaymentProvider } from '@fluxo/types'
import {
    fetchMyInvoiceById,
    downloadInvoicePDF,
    ...
    const handlePay = async () => {
        ...
        try {
            if (paymentProvider === PaymentProvider.ACCOUNT_BALANCE) {
                notifications.info(
                    'Account balance payment is not available yet'
                )
                return
            }
            ...
            if (result.payment?.completed) {
                notifications.success('Payment completed')
                const refreshed = await fetchMyInvoiceById(invoiceId)
                if (refreshed) setInvoice(refreshed)
                return
            }

            notifications.success(result.message ?? 'Payment initiated')
        } catch (error) {
