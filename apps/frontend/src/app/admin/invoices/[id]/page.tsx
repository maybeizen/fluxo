...
import { type Invoice, InvoiceStatus } from '@fluxo/types'
import {
    fetchInvoiceById,
    downloadInvoicePDF,
    updateInvoice,
...
export default function ViewInvoicePage() {
    ...
    const handlePayTest = async () => {
        ...
        try {
            ...
            notifications.success('Invoice marked as paid (test)')
            const data = await fetchInvoiceById(invoiceId)
            if (data) setInvoice(data)
        } catch (error) {
            console.error('Failed to mark invoice as paid:', error)
            notifications.error('Failed to mark invoice as paid')
        } finally {
            setIsPaying(false)
        }
    }
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        {invoice.status === InvoiceStatus.PENDING && (
                            <Button
                                ...
                                title="Simulate payment (testing only)"
                            >
                                {isPaying ? 'Marking…' : 'Pay (test)'}
                            </Button>
                        )}
                    </div>
                </div>
