...
export const userDiscord = pgTable(
    'user_discord',
    {
        id: serial('id').primaryKey(),
    ...
    (table) => ({
        userIdIdx: index('user_discord_user_id_idx').on(table.userId),
    })
)
...
export const userPasswordReset = pgTable(
    ...
    (table) => ({
        tokenIdx: index('user_password_reset_token_idx').on(table.token),
    })
)
...
export const userEmailVerification = pgTable(
    ...
    (table) => ({
        tokenIdx: index('user_email_verification_token_idx').on(table.token),
    })
)
